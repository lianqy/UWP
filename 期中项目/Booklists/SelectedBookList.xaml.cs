...
namespace Booklists
{
    ...
    /// </summary>
    public sealed partial class SelectedBookList : Page
    {
        public SelectedBookList()
        {
            ...
            using (var mydb = conn2.Prepare(@"SELECT id, fid, title, description, classification, introduction, buy_link, completed, imagebyte, date FROM item2"))
            {
                while (SQLiteResult.ROW == mydb.Step())
                {
                    if(mydb[1].ToString() == fid) ///
                    {
                        ...
                        bookViewModel.AddBook(true, mydb[0].ToString(), mydb[1].ToString(), mydb[2].ToString(), mydb[3].ToString(), mydb[4].ToString(), mydb[5].ToString(), mydb[6].ToString(), Boolean.Parse(mydb[7].ToString()), bitmapImage, mybytes, mydb[9].ToString());
                        /*
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            /*
            if (this.BooklistViewModel.SelectedItem.books != null)
            {
                for (int i = 0; i < this.BooklistViewModel.SelectedItem.books.Count; i++)
                {
                    this.bookViewModel.addbook(this.BooklistViewModel.SelectedItem.books[i]);
                }
            }
            */
            /*
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                bookViewModel.UpdateBook(Title1.Text, Description1.Text, tagbox.Text, Content1.Text, "www.baidu.com", false, bgimage.Source, imagebyte, Date1.Date.ToString());
                Title1.Text = "";
                Description1.Text = "";
                Content1.Text = "";
                Date1.Date = DateTime.Now; /////TODO
                Frame.Navigate(typeof(SelectedBookList), bookViewModel);
            }
        }
